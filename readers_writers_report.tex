    ...
    def start_read(self, thread_id):
        with self.lock:
            ...
            while True:
                ...
                me.wait()
\end{lstlisting}
\subsection{Adaptive Strategy (Aging)}
We implemented an advanced "Adaptive" lock (also known as priority inheritance or aging). By default, it operates in \textbf{Reader Priority} to maximize throughput. However, if a Writer waits longer than a defined threshold ($T > 1.0s$), the system dynamically switches to \textbf{Strict Writer Priority} to prevent starvation.
\begin{lstlisting}[language=Python]
class AdaptiveRWLock(ReadWriteLock):
    def start_read(self, thread_id):
        while True:
            # If Writer Starving -> Block new readers
